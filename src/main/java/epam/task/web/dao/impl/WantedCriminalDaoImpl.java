...
public class WantedCriminalDaoImpl implements WantedCriminalDao {
    ...
    private WantedCriminal createCriminal(ResultSet resultSet) throws SQLException {
        long guiltyId = resultSet.getLong(GUILTY_ID);
        String firstName = resultSet.getString(FIRST_NAME);
        String lastName = resultSet.getString(LAST_NAME);
        String crimCity = resultSet.getString(CRIM_CITY);
        String adress = resultSet.getString(ADRESS);
        Date DOB = resultSet.getDate(CRIM_DOB);
        BigDecimal reward = resultSet.getBigDecimal(REWARD);
        CrimType crimeType = CrimType.valueOf(resultSet.getString(CRIM_TYPE));
